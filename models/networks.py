...
def define_C(opt):
    ...
    if which_model == "Unet":
        ...
        raise NotImplementedError(f"Restoration model [{which_model}] is not recognized.")
    return netC
...
def define_D(opt):
    ...
    if which_model == "discriminator_vgg_128":
        ...
        netD_HR = SRGAN_arch.NLayerDiscriminator(
            in_nc=opt_net["in_nc"],
            nf=opt_net["nf"],
        ...
        raise NotImplementedError(f"Discriminator model [{which_model}] is not recognized.")
    return netD_HR
...
def define_Net1(opt):
    ...
    if which_model == "NAFNet":
        ...
        raise NotImplementedError(f"Stage-1 model [{which_model}] is not recognized.")
    return Net1
...
def define_Net_SR(opt):
    ...
    if which_model == "NAFNet":
        ...
        train_dataset = opt.get("datasets", {}).get("train", {})
        img_size_xy = int(train_dataset.get("GT_size_xy", 32))
        img_size_z = int(train_dataset.get("GT_size_z", 8))
        Net_SR = HAT(
            img_size=(img_size_z, img_size_xy, img_size_xy),
            upscale=scale,
            depths=opt_net["depth"],
            num_heads=opt_net["num_heads"],
            window_size=opt_net["window_size"],
            mlp_ratio=opt_net["mlp_ratio"],
            embed_dim=opt_net["n_fea"],
        )
    else:
        raise NotImplementedError(f"SR model [{which_model}] is not recognized.")
    return Net_SR
