...
def _result_label(denoise_name, sr_name, use_sr):
    if use_sr and sr_name:
        return sr_name
    if denoise_name:
        return denoise_name
    return "UnknownModel"
...
def _display_name(path_like):
    return os.path.basename(str(path_like))


def _log_retry(file_name):
    logger.warning("Retrying %s with smaller tiles after a runtime issue.", file_name)


def _profile_display_name(opt):
    sr_name = opt.get("sr_name") or ""
    dn_name = opt.get("dn_name") or ""

    if sr_name.endswith("_SR"):
        parts = sr_name.split("_")
        if len(parts) >= 3:
            return f"{parts[0]} / {parts[1]}"

    if dn_name.startswith("Denoise_"):
        organelle = dn_name[len("Denoise_"):]
        if organelle:
            return f"SDCM / {organelle}"
...
def predict_tile(model, x, net_axes_in_div_by, block_size, overlap, method="cover"):
    ...
    logger.debug(f"Pad img {x.shape} => {x_pad.shape} with pad: {pad}")
    ...
    logger.debug(f"predict start with {len(input_slice)} patches...")
    for pdx, (i_slice, o_slice, crop_slice) in enumerate(zip(input_slice, output_slice, crop_slice)):
        ...
        logger.debug(f"patch [{pdx + 1:02d}]/ [{len(input_slice)}] shape: {block.shape} time: {patch_time:.4f}s")
...
def _interpolate_z_for_sr(input_volume, z_upscale_factor, stage_tag):
    logger.debug(">>> %s: Upsampling Z-axis by factor %.6f...", stage_tag, z_upscale_factor)
    sr_input = zoom(input_volume, (z_upscale_factor, 1, 1), order=1)
    logger.debug("    Shape change: %s -> %s", input_volume.shape, sr_input.shape)
    return sr_input
...
def main(opt):
    ...
    profile_name = _profile_display_name(opt)
    ...
    for phase, dataset_opt in sorted(opt['datasets'].items()):
        ...
        logger.info('Found {:d} TIFF stack(s) for [{:s}].'.format(len(test_set), profile_name))
        logger.info('Starting inference for [{:s}]...'.format(profile_name))
        ...
        for idx, (test_data, mi, ma) in enumerate(test_loader):
            ...
            current_file_name = _display_name(tiff_file_path)
            ...
            mi = mi[0].numpy()
            ma = ma[0].numpy()
            logger.info(f"Processing [{idx + 1}/{len(test_loader)}]: {current_file_name}")

            load_data_end = time.time()
            load_data_time = load_data_end - load_data_start
            logger.debug(f"Loading data time: {load_data_time:.4f}s")
            ...
            if idx == 0:
                ...
                with open(os.path.join(save_img_base, 'label.txt'), "w") as file:
                    file.write(_result_label(opt.get('dn_name'), opt.get('sr_name'), original_sr_label))
            ...
            if is_split_mode:
                ...
                # ==========================================
                logger.debug(">>> [Split Mode] Stage 1: Running Denoise...")
                ...
                while not success:
                    try:
                        ...
                        _log_retry(current_file_name)
                        logger.debug(f"Stage 1 retry with block_size={block_size}: {e}")
                ...
                # ==========================================
                logger.debug(">>> [Split Mode] Stage 2: Running SR...")
                ...
                while not success:
                    try:
                        ...
                        failed_count = (failed_count + 1) % test_img.ndim
                        _log_retry(current_file_name)
                        logger.debug(f"Stage 2 retry with block_size={block_size}: {e}")

                task_suffix = '_SR_'
                fold_name = "SR"

            elif opt['sr_label']:
                logger.debug(">>> [SR Mode] Preparing isotropic input for SR...")
                sr_input = _interpolate_z_for_sr(
                    test_img,
                    z_upscale_factor,
                ...
                while not success:
                    try:
                        ...
                        failed_count = (failed_count + 1) % test_img.ndim
                        _log_retry(current_file_name)
                        logger.debug(f"SR retry with block_size={block_size}: {e}")
                ...
                while not success:
                    try:
                        ...
                        failed_count = (failed_count + 1) % test_img.ndim
                        _log_retry(current_file_name)
                        logger.debug(f"Denoise retry with block_size={block_size}: {e}")
            ...
            logger.info(f"Completed [{idx + 1}/{len(test_loader)}]: {current_file_name} ({tiff_time_end - tiff_time_start:.2f}s)")

        predict_time_end = time.time()
        logger.info(f"Completed dataset [{profile_name}] in {predict_time_end - predict_time_start:.2f}s")
        if 'save_img_base' in locals():
            logger.info(f"Output folder: {save_img_base}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", choices=["ILCM", "SDCM"])
    ...
    logger.info(f"Loaded config: {args.opt}")
    logger.debug("Yaml params: \n" + options.dict2str(opt))
    ...
    for key, path in opt["path"].items():
        if isinstance(path, str) and key not in {"experiments_root", "root", "repo_root"} and not list_any_in_str(["pretrain_model", "resume"], key):
            if not os.path.exists(path):
                mkdirs(path)
                logger.debug(f"Create {key} path: {path}")
