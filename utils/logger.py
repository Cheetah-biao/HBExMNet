...
_LOGGER = {}
_LOG_DIR = os.path.join(os.path.join(__file__, os.path.pardir, os.path.pardir), "logs")
...
def get_logger(logger_name="", log_dir=_LOG_DIR):
    ...
    if logger_name in _LOGGER:
        return _LOGGER[logger_name or "sys"]

    console_format = "<g>{time:YYYY-MM-DD HH:mm:ss.SSS}</g> <r>|</r> <level>{level:<8}</level><<y>{process:>5}</y>:<e>{thread:<5}</e>>" \
                     " <r>|</r> <c>{name}</c><r>:</r><c>{function}</c><r>:</r><c>{line}</c> <r>|</r> <level>{message}</level>"
    ...
    _LOGGER[logger_name] = new_logger
