...
def _normalize_config(selection=None):
    ...
    config = {
        "mode": _internal_mode(selection.get("mode", "ILCM")),
...
def _launch_gui():
    ...
    mode_var = tk.StringVar(value=_display_mode(defaults["mode"]))
    ...
    hero = _make_hero(
        main,
        "HBExMNet Training",
        "Build paired training patches from your LR and GT folders, then launch the selected ILCM or SDCM training stage with matching defaults.",
    )
    ...
    ttk.Combobox(exp_frame, textvariable=mode_var, values=["ILCM", "SDCM"], state="readonly", justify="center").grid(row=0, column=1, sticky="ew", padx=4, pady=6)
