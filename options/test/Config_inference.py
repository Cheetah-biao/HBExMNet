...
from utils.project_paths import workspace_path, find_external_data_dir, find_external_or_repo_dir


EXPERIMENTS_ROOT = find_external_or_repo_dir("experiments")
...
DEFAULT_PROFILE_PRESETS = {
    ("ILCM", "Tub"): {"xy_nm": 59.0, "z_nm": 59.0, "denoise": False},
    ("ILCM", "Rab7"): {"xy_nm": 59.0, "z_nm": 59.0, "denoise": False},
    ("ILCM", "Tomm20"): {"xy_nm": 59.0, "z_nm": 59.0, "denoise": False},
...
def parse_model_name(full_name):
    if full_name.startswith("Denoise_"):
        organelle = full_name[len("Denoise_"):]
        if organelle in SUPPORTED_ORGANELLES:
            return "SDCM", organelle, "Denoise", full_name
        return None

    if full_name.endswith("_SR"):
        stem = full_name[: -len("_SR")]
        parts = stem.split("_", 1)
        if len(parts) != 2:
            return None
        mode, organelle = parts
        if mode in {"ILCM", "SDCM"} and organelle in SUPPORTED_ORGANELLES:
            return mode, organelle, "SR", full_name
    return None


def _display_mode(internal_mode: str) -> str:
    return internal_mode


def _internal_mode(display_mode: str) -> str:
    return display_mode


def _pipeline_flags(mode: str):
    internal_mode = _internal_mode(mode)
    if internal_mode == "SDCM":
        return True, True
    return False, True
...
def _latest_denoise_model(models, organelle):
    candidates = [entry for entry in models if entry.task == "Denoise" and entry.organelle == organelle]
    if not candidates:
        return None
    return max(candidates, key=lambda entry: entry.modified_time)


def _workflow_summary(mode: str, organelle: str, models) -> str:
    internal_mode = _internal_mode(mode)
    sr_entry = _latest_matching_model(models, internal_mode, organelle, "SR")
    run_denoise, _run_sr = _pipeline_flags(internal_mode)
    denoise_entry = _latest_denoise_model(models, organelle) if run_denoise else None

    profile_line = f"Profile : {_display_mode(internal_mode)} / {organelle}"
    if run_denoise:
        bundle_ready = denoise_entry is not None and sr_entry is not None
        if denoise_entry and sr_entry:
            asset_stem = f"{_display_asset_name(denoise_entry.folder_name)} + {_display_asset_name(sr_entry.folder_name)}"
        else:
            asset_stem = f"Denoise_{organelle} + SDCM_{organelle}_SR"
    else:
        bundle_ready = sr_entry is not None
        asset_stem = _display_asset_name(sr_entry.folder_name) if sr_entry else f"{_display_mode(internal_mode)}_{organelle}"

    status_line = f"Bundle  : {'ready' if bundle_ready else 'missing required asset(s)'}"
    asset_line = f"Asset   : {asset_stem}"
    return "\n".join([profile_line, status_line, asset_line])


def _default_profile(mode: str, organelle: str):
    return DEFAULT_PROFILE_PRESETS.get((_internal_mode(mode), organelle), {"xy_nm": 65.0, "z_nm": 65.0, "denoise": False})
...
def _normalize_selection(selection=None):
    ...
    mode = _internal_mode(selection.get("mode", ""))
    ...
    if z_zoom is None:
        _, run_sr = _pipeline_flags(mode)
        z_zoom = float(z_nm) / float(xy_nm) if run_sr else 1.0
    return {
        "mode": mode,
...
def _resolve_selection(selection=None):
    ...
    if not normalized["mode"]:
        available_modes = [mode for mode in ("ILCM", "SDCM") if mode in {entry.mode for entry in models}]
    ...
    run_denoise, run_sr = _pipeline_flags(normalized["mode"])
    ...
    sr_name = None
    if run_denoise:
        entry = _latest_denoise_model(models, normalized["organelle"])
        if entry is None:
            raise FileNotFoundError(
                f"No Denoise_{normalized['organelle']} experiment was found under {EXPERIMENTS_ROOT}."
            )
...
def _launch_gui():
    ...
    preferred_modes = ["ILCM", "SDCM"]
    available_modes = [_display_mode(mode) for mode in preferred_modes if mode in {entry.mode for entry in models}]
    if not available_modes:
        available_modes = sorted({_display_mode(entry.mode) for entry in models})

    mode_var = tk.StringVar(value="ILCM" if "ILCM" in available_modes else (available_modes[0] if available_modes else ""))
    ...
    def organelle_options():
        internal_mode = _internal_mode(mode_var.get())
        run_denoise, run_sr = _pipeline_flags(internal_mode)
        organelles = []
        for organelle in sorted({entry.organelle for entry in models if entry.mode == internal_mode}):
            if run_denoise and _latest_matching_model(models, internal_mode, organelle, "Denoise") is None:
                continue
            if run_sr and _latest_matching_model(models, internal_mode, organelle, "SR") is None:
                continue
    ...
    def selected_model_summary():
        organelle = organelle_var.get()
        if not organelle:
            return "Profile : not selected\nBundle  : waiting\nAsset   : waiting"
        return _workflow_summary(mode_var.get(), organelle, models)
    ...
    def refresh_status(*_args):
        try:
            ...
            if xy_nm <= 0 or z_nm <= 0:
                raise ValueError
            _run_denoise, run_sr = _pipeline_flags(mode_var.get())
            computed_zoom = z_nm / xy_nm if run_sr else 1.0
    ...
    hero = _make_hero(
        main,
        "HBExMNet Inference",
        "Choose one TIFF folder to restore. The GUI loads the default voxel size and processing pipeline from the selected microscope mode and organelle.",
    )
    ...
    ttk.Label(profile_frame, text="Resolved profile", style="Card.TLabel").grid(row=2, column=0, sticky="nw", padx=4, pady=(10, 4))
    ...
    ttk.Label(
        sampling_frame,
        text="Defaults follow the bundled data sheet: ILCM uses the bundled direct super-resolution workflow, while SDCM uses the bundled live-cell restoration workflow with the matched voxel size preset.",
        style="PanelMuted.TLabel",
        wraplength=360,
        justify=tk.LEFT,
    ...
    for variable in (organelle_var, input_path_var, xy_nm_var, z_nm_var):
...
if __name__ == "__main__":
    ...
    print("\n--- GUI Selections ---")
    print(f"Selected stage-1 asset: {denoise_name}")
    print(f"Selected stage-2 asset: {sr_name}")
