...
- pretrained models stay under a resolved `experiments/` folder
...
- The pretrained model package must be extracted under an `experiments/` folder that the launcher can resolve. In this local release, the default model root is the sibling folder `experiments/` beside `HBExMNet`.
...
The GUI resolves bundled models from `experiments/`. Test data does not need to be placed inside the project folder.
...
- `Mode`: `ILCM` or `SDCM`
- `Organelle`
- `ILCM` defaults to `XY = 59 nm`, `Z = 59 nm`, and uses the bundled direct super-resolution workflow
- `SDCM` automatically uses `Denoise_<Organelle>` followed by `SDCM_<Organelle>_SR`
...
- the processing workflow is fixed by the selected microscope mode
...
- The final pretrained model layout is `ILCM_<Organelle>_SR`, `SDCM_<Organelle>_SR`, and `Denoise_<Organelle>`.
